...
1. ~~网页版三个新能力已实现并通过回归测试~~ **已满足**
   （`only_systems` + 上传解析预览 + `report_date`，回归测试覆盖）
2. ~~本地 `LOCAL_MODE` 跑通，浏览器上传可用~~ **已满足**（端到端实测通过）
3. `run.py` 的批量/脚本化场景确认已被覆盖或明确放弃 —— **尚未满足**

第 3 条的具体缺口：`run.py` 可以直接指定一个本地目录生成报告，不经过 HTTP 上传，
因此不受 `MAX_UPLOAD_BYTES`（默认 200MB）限制。网页版目前必须走上传。
日志包很大、或需要脚本化定时跑时，这条路径仍有价值。

在补上等价能力（例如一个读本地目录的 CLI 入口）之前，先留着。
