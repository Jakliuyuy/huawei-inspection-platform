...
Sub ProcessTab(ByRef tabObj, ByVal logRoot, ByRef summaryStream)
    ...
    Dim screenObj, promptText, logPath, i, cmdArr, found, deviceName, loginMatchIdx

    If Not IsTabConnected(tabObj) Then
        If Not EnsureTabConnected(tabObj, summaryStream, GetTabCaptionSafe(tabObj)) Then
            summaryStream.WriteLine "[" & Now & "] Skip (not connected after retries): " & GetTabCaptionSafe(tabObj)
            Exit Sub
        End If
    End If
    ...
    loginMatchIdx = WaitForLoginPrompt(screenObj, 10)
    If loginMatchIdx = 0 Then
    ...
    ' Disable Pager
    SendCommandWithRecoveryTOC tabObj, screenObj, promptText, logPath, pagerCommand, 10, summaryStream, deviceName

    ' Run TOC Commands
    cmdArr = Split(commands, ";;")
    For i = 0 To UBound(cmdArr)
        If Len(Trim(cmdArr(i))) > 0 Then
            SendCommandWithRecoveryTOC tabObj, screenObj, promptText, logPath, cmdArr(i), 120, summaryStream, deviceName ' Longer timeout for license/alarm cmds
        End If
    Next
...
Function EnsureTabConnected(ByRef tabObj, ByRef summaryStream, ByVal contextText)
    Dim attempt, waitStep, connectError
    EnsureTabConnected = False
    If IsTabConnected(tabObj) Then EnsureTabConnected = True : Exit Function
    For attempt = 1 To 3
        summaryStream.WriteLine "[" & Now & "] " & contextText & " reconnect attempt " & attempt & "/3"
        connectError = ""
        On Error Resume Next
        Err.Clear
        tabObj.Session.Connect
        If Err.Number <> 0 Then connectError = Err.Description
        Err.Clear
        On Error GoTo 0
        For waitStep = 1 To (attempt * 10)
            If IsTabConnected(tabObj) Then
                summaryStream.WriteLine "[" & Now & "] " & contextText & " reconnect succeeded on attempt " & attempt
                EnsureTabConnected = True
                Exit Function
            End If
            crt.Sleep 500
        Next
        If Len(connectError) > 0 Then summaryStream.WriteLine "[" & Now & "] " & contextText & " reconnect error: " & connectError
    Next
End Function

Function RestoreTOCSession(ByRef tabObj, ByRef screenObj, ByRef prompt, ByVal logPath, ByRef summaryStream, ByVal deviceName)
    Dim matchIdx, newPrompt
    RestoreTOCSession = False
    If Not EnsureTabConnected(tabObj, summaryStream, deviceName) Then Exit Function
    tabObj.Activate
    Set screenObj = tabObj.Screen
    screenObj.Synchronous = True
    If HasVisiblePasswordDecisionPrompt(screenObj) Then
    ...
    Else
        screenObj.Send vbCr
    End If
    matchIdx = WaitForLoginPrompt(screenObj, 15)
    If matchIdx = 0 Then
        summaryStream.WriteLine "[" & Now & "] " & deviceName & " reconnect login prompt not detected"
        Exit Function
    End If
    newPrompt = DetectPromptText(screenObj)
    If Len(newPrompt) > 0 Then prompt = newPrompt
    StartSessionLog tabObj, logPath
    summaryStream.WriteLine "[" & Now & "] " & deviceName & " session restored, retrying current command"
    RestoreTOCSession = True
End Function

Function SendCommandWithRecoveryTOC(ByRef tabObj, ByRef screenObj, ByRef prompt, ByVal logPath, ByVal cmd, ByVal timeout, ByRef summaryStream, ByVal deviceName)
    Dim attempt
    SendCommandWithRecoveryTOC = False
    For attempt = 1 To 3
        If Not IsTabConnected(tabObj) Then
            If Not RestoreTOCSession(tabObj, screenObj, prompt, logPath, summaryStream, deviceName) Then Exit Function
        End If
        If SendCommand(screenObj, prompt, cmd, timeout) Then
            SendCommandWithRecoveryTOC = True
            Exit Function
        End If
        If attempt < 3 Then
            summaryStream.WriteLine "[" & Now & "] " & deviceName & " command retry " & (attempt + 1) & "/3: " & cmd
            If IsTabConnected(tabObj) Then
                On Error Resume Next
...
Function SendCommand(ByRef screenObj, ByVal prompt, ByVal cmd, ByVal timeout)
    ...
    If Err.Number <> 0 Then Err.Clear : SendCommand = False
    On Error GoTo 0
End Function
...
Function WaitForPrompt(ByRef screenObj, ByVal prompt, ByVal timeout, ByVal retries)
    ...
    Do
        ...
        If Err.Number <> 0 Then Err.Clear : On Error GoTo 0 : WaitForPrompt = False : Exit Function
        On Error GoTo 0
        If res = 0 Then
            count = count + 1
        ...
        ElseIf res >= 1 And res <= 3 Then
            screenObj.Send " "
        ElseIf res = 4 Then
            ' Handle [Y/N]: with the same reliable NO sequence used during login.
            SendPasswordDecisionNo screenObj
            count = 0 
        Else
