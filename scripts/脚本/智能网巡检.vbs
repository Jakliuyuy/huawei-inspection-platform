...
Sub Main()
    ...
    summaryPath = g_fso.BuildPath(logRoot, "IntelligentNet_summary_" & runStamp & ".log")
    Set summaryStream = g_fso.OpenTextFile(summaryPath, ForWriting, True)
    summaryStream.WriteLine "IntelligentNet daily check started: " & Now
    ...
    summaryStream.WriteLine String(72, "=")
    summaryStream.WriteLine "IntelligentNet daily check finished: " & Now
    summaryStream.Close
...
Sub RunCheckForTab(ByRef tabObj, ByVal logRoot, ByRef summaryStream)
    ...
    logPath = g_fso.BuildPath(logRoot, "IntelligentNet_" & Replace(deviceName, " ", "_") & "_" & TimeStampForFile(Now) & ".log")
...
Function GetCommandsForSession(ByVal sessionName)
    Dim normalizedName
    normalizedName = UCase(Trim(sessionName))

    If normalizedName = "XZYD-VPN-RT01" Or normalizedName = "XZYD-VPN-RT02" Then
        GetCommandsForSession = Array( _
            "display version", _
            "display device manuinfo", _
            "display startup", _
            "display cpu-usage", _
