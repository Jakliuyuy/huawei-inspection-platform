...
Sub Main()
    ...
    summaryPath = g_fso.BuildPath(logRoot, "SMS_summary_" & runStamp & ".log")
    Set summaryStream = g_fso.OpenTextFile(summaryPath, ForWriting, True)
    summaryStream.WriteLine "SMS daily check started: " & Now
    ...
    summaryStream.WriteLine String(72, "=")
    summaryStream.WriteLine "SMS daily check finished: " & Now
    summaryStream.Close
...
Sub RunCheckForTab(ByRef tabObj, ByVal logRoot, ByRef summaryStream)
    ...
    logPath = g_fso.BuildPath(logRoot, "SMS_" & Replace(deviceName, " ", "_") & "_" & TimeStampForFile(Now) & ".log")
...
Function GetCommandsForSession(ByVal sessionName)
    ...
    ' �������豸 (����ǰ׺Ϊ XZYD)
    If InStr(nameUpper, "XZYD") > 0 Then
        GetCommandsForSession = Array( _
            "show version", _
            "show startup-config", _
            "show alarm", _
            "show cpu", _
            "show cpu core", _
            "show memory", _
            "show memory slot", _
            "show interfaces status", _
            "show ip interface brief", _
            "show ip route", _
            "show logging" _
        )
    ' ��Ի�Ϊ�豸 (LSJZ, LZLS ��)
