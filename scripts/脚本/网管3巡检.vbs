...
Sub Main()
    ...
    summaryPath = g_fso.BuildPath(logRoot, "NM3_summary_" & runStamp & ".log")
    Set summaryStream = g_fso.OpenTextFile(summaryPath, ForWriting, True)
    summaryStream.WriteLine "NetMgmt-3 daily check started: " & Now
    ...
    summaryStream.WriteLine String(72, "=")
    summaryStream.WriteLine "NetMgmt-3 daily check finished: " & Now
    summaryStream.Close
...
Sub RunCheckForTab(ByRef tabObj, ByVal logRoot, ByRef summaryStream)
    ...
    logPath = g_fso.BuildPath(logRoot, "NM3_" & Replace(deviceName, " ", "_") & "_" & TimeStampForFile(Now) & ".log")
